...
process.on('uncaughtException', (error) => {
  console.error(`Image thumbnail backfill failed: ${formatError(error)}`);
  process.exit(1);
});
...
const scanLimit = readPositiveInt(args.scanLimit, Math.max(limit * 5, 100));
const maxWidth = readPositiveInt(args.maxWidth, 720);
const quality = readPositiveInt(args.quality, 4);
...
if (posts.length === 0) {
  console.log('No image posts need generated thumbnails.');
  process.exit(0);
}

console.log(
  `Image thumbnail backfill: ${dryRun ? 'dry-run' : 'apply'} mode, ` +
    `${posts.length} candidate(s), limit ${limit}, scanned ${scanLimit}.`,
);
...
for (const post of posts) {
  ...
  if (!plan) {
    skipped += 1;
    console.log(`SKIP ${post.id}: media_url does not expose a usable image key.`);
    continue;
  }
  ...
  console.log(`  old thumb: ${post.thumbnail_url ? stripQuery(post.thumbnail_url) : '(empty)'}`);
  ...
  const tempDir = await fs.promises.mkdtemp(path.join(os.tmpdir(), 'vibes-image-thumb-'));
...
async function loadCandidatePosts() {
  ...
  return (data ?? [])
    .filter(needsGeneratedThumbnail)
    .slice(0, limit);
}

function needsGeneratedThumbnail(post) {
  const mediaUrl = safeUrl(post.media_url);
  if (!mediaUrl) return false;
  const mediaKey = decodeR2Key(mediaUrl.pathname);
  if (!mediaKey?.startsWith('posts/images/')) return false;

  const thumbnailUrl = safeUrl(post.thumbnail_url);
  if (!thumbnailUrl) return true;

  const thumbnailKey = decodeR2Key(thumbnailUrl.pathname);
  return !thumbnailKey || thumbnailKey === mediaKey || thumbnailKey.startsWith('posts/images/');
}
...
function buildThumbnailPlan(post) {
  ...
  const keyFromMedia = decodeImageKeyFromMediaPath(mediaUrl.pathname);
  if (!keyFromMedia) return null;

  const ownerId = keyFromMedia.ownerId ?? post.author_id;
  const key = `thumbnails/${ownerId}/${safeKeySegment(keyFromMedia.stem)}.jpg`;
...
function decodeImageKeyFromMediaPath(pathname) {
  ...
  if (mediaKind !== 'images' || !ownerId || !filename) return null;
...
function parseArgs(rawArgs) {
  ...
  for (let i = 0; i < rawArgs.length; i += 1) {
    ...
    else if (arg === '--quality') parsed.quality = rawArgs[++i];
...
function printHelp() {
  console.log(`
Usage:
  npm run images:thumbnails:backfill -- [options]

Options:
  --dry-run              List candidate image posts and planned thumbnail URLs (default)
  --apply                Generate, upload and write thumbnail_url
  --limit <n>            Number of candidate posts to process (default: 25)
  --scan-limit <n>       Number of recent image posts to inspect (default: max(limit*5, 100))
  --post-id <uuid>       Restrict to one post; can be repeated
  --max-width <pixels>   Max generated JPEG width (default: 720)
  --quality <n>          ffmpeg JPEG q:v value, lower is better (default: 4)
