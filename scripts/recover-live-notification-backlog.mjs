...
console.log('Live notification backlog recovery');
...
if (result) {
  ...
  if (!execute && Number(result.matched || 0) > 0) {
    console.log('');
    console.log('Dry run only. Re-run with --execute to mark these old live notifications as read.');
  }
}

if (failures.length > 0) {
  console.log('');
  console.error('Live notification backlog recovery failed:');
  for (const failure of failures) console.error(`  - ${failure}`);
  process.exit(1);
}

console.log('');
console.log('Live notification backlog recovery check passed.');

async function callRecoveryRpc() {
  const response = await fetchWithTimeout(`${supabaseUrl}/rest/v1/rpc/live_notification_backlog_recovery`, {
    method: 'POST',
    headers: {
  ...
  if (!response.ok) {
    failures.push(`[rpc] live_notification_backlog_recovery failed: ${response.status} ${summarize(text)}`.trim());
    return null;
  }
  try {
    return JSON.parse(text);
  } catch {
    failures.push('[rpc] live_notification_backlog_recovery returned invalid JSON.');
    return null;
  }
}
...
function printHelp() {
  console.log(`
Usage: node scripts/recover-live-notification-backlog.mjs [options]

Dry-runs by default. Use --execute only after a product/ops decision.

Options:
  --older-than-days <n>  Target unread live notifications older than n days (default ${DEFAULT_OLDER_THAN_DAYS})
  --limit <n>            Maximum rows to update/check in one run (default ${DEFAULT_LIMIT})
  --execute              Mark matching old live notifications as read
