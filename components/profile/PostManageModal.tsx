...
import { Pencil, Trash2, BarChart2, Pin, PinOff } from 'lucide-react-native';
...
type PostStats = {
  likes: number;
  comments: number;
  resonanz: number; // dwell_time_score × 100
};
...
export function PostManageModal({
  ...
  return (
    <Modal visible={visible} transparent animationType="fade">
      <Pressable style={s.modalOverlay} onPress={() => { onClose(); setStatsOpen(false); setStats(null); }}>
        <Pressable style={s.modalContent} onPress={(e) => e.stopPropagation()}>
          ...
          <Pressable
            style={s.modalItem}
            onPress={() => { onClose(); setStatsOpen(false); setStats(null); onEdit(); }}
          >
          ...
          {onTogglePin && (
            <Pressable
              style={s.modalItem}
              onPress={() => { onClose(); setStatsOpen(false); setStats(null); onTogglePin(); }}
            >
              {isPinned
          ...
          <Pressable
            ...
            onPress={() => { onClose(); setStatsOpen(false); setStats(null); onDelete(); }}
          >
