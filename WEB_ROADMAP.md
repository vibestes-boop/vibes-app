...
### v1.w.12.8 — Web-Push Fan-Out-Parity über Notifications-Hub (2026-04-22)

**Scope**: Schließt den letzten Asymmetrie-Gap zwischen Native-Push und Web-Push. Bisher kam auf `serlo-web.vercel.app` ausschließlich für DMs eine Browser-Notification an (v1.w.12.4-Trigger auf `messages` INSERT). Follow/Like/Comment/Live/Live-Invite/Gift/Scheduled-Live-Reminder/New-Order gingen ausschließlich als Expo-Push an die Native-App — Web-User mit aktiver Subscription bekamen für diese 8 Typen still nichts. Jetzt fließen alle 9 Notification-Typen durch beide Kanäle.

**Architektur-Entscheidung — Path B4 (Edge-Function-Extension) statt Path A (parallele DB-Trigger)**: Die naheliegende Option wäre gewesen, parallel zum bestehenden `notify_web_push_on_dm` jeweils einen Trigger auf `follows`, `gift_transactions`, `live_sessions` (+ Follower-Fan-Out), `likes`, `comments`, Shop-Orders zu schreiben — fünf bis sechs neue Trigger-Funktionen. Verworfen, weil alle diese Tabellen-Inserts bereits sekundär-wirksam eine Zeile in `notifications` erzeugen (teils via eigenem Trigger, teils inline im RPC), und die bestehende `fn_send_push_on_notification` schon den gesamten Fan-Out-Punkt darstellt. Anstelle dessen fängt `send-push-notification` Edge Function jetzt NACH der Expo-Dispatch zusätzlich `send-web-push` auf — ein zentraler Ort, eine Formatierungslogik (die bestehende `messages`-Dict), single Source of Truth für Title/Body.

**Entscheidung gegen das v1.w.12.6-Warte-Gate**: v1.w.12.6 hatte explizit entschieden, das Fanout-Gate („erst nach Production-Metrics dass DM-Push sauber zustellt") einzuhalten. Hier bewusst aufgehoben, weil Path B4 das Risiko-Profil fundamental ändert: Der kritische Pfad (Expo-Push) bleibt 1:1 unverändert — die Web-Push-Erweiterung läuft in einem `try/catch` NACH `await pushRes.json()` und ist fire-and-forget; jede Web-Push-Regression führt nur zu „Web bekommt nichts" (= Status quo), nie zu „Native bekommt nichts mehr". Die 410/404-Unsubscribe-Hygiene in `send-web-push` ist bereits live (v1.w.12.4), Subscription-Stale-Filter (60-Tage) auch. Ein Parallel-Trigger-Fanout-Szenario (Path A) hätte diese Garantie NICHT — jeder neue Trigger hätte eigene Fehlermodi + eigene Stale-Locks.

**DM-Double-Send-Guard**: Kritischer Design-Punkt. DMs landen heute ZWEIMAL in `notifications` bzw. dispatchen zweimal Web-Push, wenn nicht geguardet: (a) `messages` INSERT → `notify_web_push_on_dm` → `send-web-push` (direkt, v1.w.12.4), (b) DM-Notification wird zusätzlich als `notifications`-Row mit `type='dm'` erzeugt (via `notifications_extend.sql` `insert_dm_notification`-Trigger) → `trg_push_notification` → `send-push-notification`. Wenn `send-push-notification` jetzt blind an `send-web-push` weiterreicht, gäbe es TWO Web-Push-Requests pro DM → Doppel-Ping im Browser-Tray. Deshalb expliziter `if (record.type !== 'dm') { ... }` Guard im neuen Fan-Out-Block mit ausführlichem Inline-Kommentar der den Grund und die v1.w.12.4-Referenz dokumentiert. Für Expo-Push existiert dieses Problem nicht — Expo dedupliziert implizit über das `push_token`, DMs haben nur einen Notification-Eintrag der durch `trg_push_notification` läuft.

**Deep-Link-Derivation — Path-Matching mit tatsächlichem App-Router**: Vorsichtige Realitätsprüfung gegen `apps/web/app/**/page.tsx` verhindert drei Dead-Links die beim blinden Guessing passiert wären. (a) Likes/Comments → nicht `/post/[id]` (existiert nicht) sondern `/p/[postId]`. (b) Follow/Follow-Request → nicht `/u/[sender_id]` (würde UUID an die `/u/[username]`-Route geben, die intern auf Username sucht → 404) sondern `/u/[actorName]` — der bereits aufgelöste Username aus der Expo-Path-Actor-Query. (c) New-Order → nicht `/shop/my-shop` (existiert nicht) sondern `/studio/orders` (der echte Seller-Orders-Screen). Gift-URL ist zwei-geteilt: mit `session_id` → Live-Room, ohne → Sender-Profil (Shop-Gift-Kontext).

**Tag-Derivation für Browser-Grouping**: Browser-Notification-API gruppiert Pushes mit gleichem `tag` und ersetzt alte durch neue im Tray. Verwendet um Spam zu vermeiden: 10 Likes auf denselben Post in 30 Sekunden → nur EIN Tray-Entry (letzter ersetzt vorherige) statt zehn. Tag-Schema: `{type}:{entity-id}` wo `entity-id` der Kontext ist, der die „Same-Event"-Semantik definiert — `like:${post_id}`, `follow:${sender_id}`, `live:${session_id}`, `gift:${sender_id}:${session_id}`, `new_order:${post_id}`. Fallback `unknown` wenn Spalten leer (defensive — sollte nicht vorkommen, aber kein Crash-Pfad).

**Fallthrough-Verhalten für `follow_request`**: Type existiert in `notifications_extend.sql` (line 134), wird aber NICHT im Expo-`messages`-Dict formatiert → Expo-Pfad fällt heute auf „Neue Aktivität auf Vibes" Default-Body. Web-Path erbt dieses Fallthrough automatisch (identischer `msg`-Resolve-Punkt), liefert aber einen korrekten Deep-Link (`/u/[actorName]` dank Case-Clause). Kein Scope-Creep diese Lücke hier zu fixen — die Expo-messages-Dict-Erweiterung wäre ein eigener Slice (Trivial-Add, aber konzeptionell nicht in den Web-Push-Scope).

- [x] **Code-Touchpoint ist exakt EINE Datei**: `supabase/functions/send-push-notification/index.ts`. Nach `console.log('[push] Expo response: ...')` beginnt ein neuer Block mit dem DM-Skip-Guard, einem Inline-`fetch()` an `${SUPABASE_URL}/functions/v1/send-web-push` mit Bearer `SUPABASE_SERVICE_ROLE_KEY` (die Function ist bereits service-role-auth-geguarded), Payload aus dem schon aufgelösten `msg.title`/`msg.body` plus den beiden neuen Derivation-Helpern. Nach dem Expo-Response-Return folgt am Modul-Ende `deriveWebUrl(record, actorName)` und `deriveWebTag(record)` als pure Functions — keine State-Mutation, kein Side-Effect.

- [x] **Keine DB-Migration, keine neuen Secrets, keine Schema-Änderung**. Der Fan-Out nutzt die identische Auth-Schicht (`SERVICE_ROLE_KEY`), den identischen `user_id`-Input (`record.recipient_id`), identische Title/Body-Strings. Das macht den Blast-Radius bei Rollback minimal: `supabase functions deploy send-push-notification` mit dem alten File-State re-deployt und der Fan-Out verschwindet — keine verwaiste Migration, keine übrig gebliebenen Trigger, keine Schema-Debt.

- [x] **Fehler-Pfade**: Drei Isolations-Schichten. (a) Outer `try/catch` um die Expo-Dispatch bleibt der primäre Fehler-Boundary — unverändert. (b) Inner `try/catch` um den Web-Push-Call schluckt alle Exceptions, loggt als `console.warn('[push] Web-Push dispatch failed (non-fatal): ...')`. (c) `await webPushRes.json().catch(() => ({}))` schützt gegen korrupte Response-Bodies (z.B. HTML-Error-Page von Supabase-Infra bei Cold-Start-Timeout) — json-parse-fail löst keine Exception aus. Net-Effekt: unabhängig was `send-web-push` tut oder nicht tut, `send-push-notification` kehrt mit `{ ok: true, result }` HTTP 200 zurück genau wie vor dem Slice.

- [x] **Typecheck**: `npx tsc --noEmit` in `apps/web/` → 6 pre-existing Errors (1× in `lib/data/__tests__/feed.test.ts` null-Assignment, 5× in `shared/schemas/*.ts` wegen fehlender `zod`-Type-Discovery beim Monorepo-TS-Setup). **0 neue Errors aus diesem Slice**, was erwartbar ist weil der Code-Touchpoint außerhalb von `apps/web/` liegt — Edge-Functions laufen im Deno-Runtime, Deno-Errors (`Cannot find name 'Deno'`, `esm.sh`-Imports) sind im Supabase-Deploy-Path gültig und werden dort aufgelöst.

- [x] **Bewusst NICHT in v1.w.12.8**:
  - **Go-Live-Follower-Fan-Out Batching-Review**: Der bestehende `notify_on_go_live`-Trigger schreibt eine `notifications`-Row pro Follower (Loop in SQL). Bei einem 10k-Follower-Host feuert `trg_push_notification` also 10k-mal → 10k Edge-Function-Invocations → 10k `send-web-push`-Calls aus dem neuen Fan-Out. Das war auch schon vor v1.w.12.8 das Lastprofil des Expo-Paths; der Web-Path erbt es. `send-web-push` akzeptiert `user_ids: []`-Batching — aber der Trigger hier schickt `user_id` single. Batching-Migration wäre ein eigenständiges Performance-Thema (v1.w.12.9-Kandidat), wenn Metrics das überhaupt als Problem zeigen. Heute: Expo schluckt das seit Wochen, also schluckt Web es auch.
  - **In-App-Notification-Center-Sync**: Web-Push ist nur der „Tray-Ping". Die visuelle In-App-Notifications-Liste auf `/notifications` (falls existent) oder als Bell-Icon-Popover zieht ihre Daten aus derselben `notifications`-Tabelle — hat also automatisch Parität, braucht kein neues Hook-Wiring. Nur falls das Web-UI diese Liste noch nicht rendert, wäre das ein separater UI-Slice (nicht Teil dieses Push-Slices).
  - **`follow_request`-Body im Expo-`messages`-Dict ergänzen**: Trivial-Add-Scope, aber konzeptionell nicht Teil von „Web-Push Parity". Eigener One-Liner-Slice wenn gewollt.
  - **Service-Worker-Notification-Click-Handler-Audit**: Der SW muss den `url` aus der Payload tatsächlich öffnen (`clients.openWindow`). Das lief für DMs seit v1.w.12.4 — wenn die neuen Deep-Links (z.B. `/p/[postId]`) dort nicht sauber geöffnet werden, wäre das im existierenden SW-Code zu fixen, nicht hier. Als Verify-Step im ersten Live-Test zu beobachten.

**Deploy-Command**: `supabase functions deploy send-push-notification` (einmalig, ohne DB-Migration, ohne Secrets-Rotation).

**Rollback-Plan**: Alten File-State aus `git show HEAD~1:supabase/functions/send-push-notification/index.ts` wiederherstellen → redeploy. 5-Sekunden-Rollback.
