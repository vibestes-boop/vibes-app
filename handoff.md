...
- **🔴 OFFENE PUNKTE (nach Priorität):**
  ...
  5. ✅ **Web-Sammelbestellungs-Karte LIVE** (Commit `339a6cd`, Migration `20260716110000` ausgeführt). `serlo.ch/shop/[id]` + `/shop` zeigen die Fireside-Runde anonymen WhatsApp-Neukunden (anon verifiziert). Fireside-Deadline verlängert auf **25.07.** (per SQL, verifiziert).
  6. ✅ **Verlängern-Knopf** (Migration `20260716120000` ausgeführt + verifiziert). Runde in der App verlängerbar (Mein Shop → 📦 → fulfillment).
  7. ✅ **WOZ Zugang geprüft** (Migration `20260716130000` ausgeführt 16.7.; Anon-Ebene verifiziert: request/queue-RPCs permission denied, Antrags-Tabelle nicht anon-lesbar). Selbstdeklaration → Antrag → Admin-Freigabe unter **`/admin/women-only`**. ⏳ **Zaur: End-to-End auf Gerät** (nach 2. Kaltstart): Beitreten → muss „Antrag in Prüfung" zeigen (nicht Sofort-Zugang) → in /admin/women-only freigeben → Zugang. Beweist den Trigger-Schutz für eingeloggte User.
  8. ⚠️ **Admin-Echtheits-Audit (17.7.):** Alle Dashboard-Daten sind ECHT (jede Kachel bis zur DB-Quelle verifiziert); 3 Header-Attrappen gefixt (24h-Chip raus, Glocke → echter Reports-Link mit bedingtem Punkt, totes Menu-Icon raus, Commit `44776c4`). 🔴 OFFEN: **8 Health-Snapshot-RPCs sind anon-ausführbar** (interne Metriken MAU/Kosten öffentlich abrufbar) — bewusst, weil CI-Health-Skripte den anon-Key nutzen. Sauberer Fix: RPCs auf Service-Role + CI auf Service-Key-Secret umstellen (braucht neues GitHub-Secret von Zaur).
  9. **Später/an Bedingungen geknüpft:** Stripe Live (wartet auf Anwalt für AGB/Widerruf) · Supabase Custom Domain `auth.serlo.ch` für den iOS-Login-Dialog (~10 $/Mon, Checkliste `docs/auth-setup.md` §5, ⚠️ storageKey-Falle) · Coin-Shop-Reaktivierung (IAPs neu in ASC + Flag + Receipt-Verify) · **WOZ Stufe 2 optional: Community-Bürgschaft** (verifizierte Frauen bürgen für Neue — später, wenn vertrauenswürdiger Kern da; heute admin-approval korrekt) · Marktplatz Phase 2 (Connect/KYC) · Kleinkram: Telegram-Alert-Token rotieren, `bunny-webhook` in config.toml prüfen, Resend/E-Mail (Android-Signup).
...
| **DB-Migrationen** | ✅ **ALLE ausgeführt** (zuletzt 16.7.: `20260716100000_live_sessions_woz_rls_fix` — WOZ-Live-Leak zu, Anon-Probe verifiziert `[]`). ⚠️ `schema_live.sql`-Dump zeigt noch die alte Policy — bei nächster Gelegenheit neu dumpen. |
...
| **🔴 NÄCHSTE AUFGABE** | **(1) ✅ kaputter Post gelöscht** — stability-Checks lokal grün (16.7.). **(2) Zaur: 2 Moderations-Meldungen über SLA** (älteste ~10 Tage!) im Command-Center abarbeiten. **(3)** testen + verkaufen (Parfüm-Funnel). **(4)** ce-Excel → ce.ts (wenn geliefert). **(5)** Google-Branding-Prüfung checken. |
