...
- **🔴 OFFENE PUNKTE (nach Priorität):**
  ...
  5. 🔴 **OFFENE MIGRATION `20260716110000_active_preorder_round_public`** (Zaur im SQL-Editor ausführen) — anon-taugliche RPC für die neue Web-Sammelbestellungs-Runden-Karte (Commit `339a6cd`). Danach zeigt `serlo.ch/shop/[id]` + `/shop` die laufende Fireside-Runde auch anonymen WhatsApp-Neukunden. Verifikation danach: `curl -s .../rest/v1/rpc/get_active_preorder_round_public -X POST -H "apikey: <anon>" -d '{}'` muss die Runde liefern (nicht permission-denied).
