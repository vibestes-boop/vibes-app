...
> - **✅ Binary v1.31.0 (Build 292) EINGEREICHT** (12.7., „Warten auf Prüfung", Auto-Release) — einzige native Neuerung: `expo-localization`. **Inkl. russischem Store-Listing** (Beschreibung/Notes/Keywords/Untertitel; Datenschutz-/Support-URL auf www.serlo.ch). ⚠️ ru-App-Name ist **`Serlo: комьюнити и эфиры`** („Serlo" im ru-Locale fremdbelegt — ASC-Namen sind pro Sprache einzigartig; Memory `vibes-asc-locale-gotchas`: neue Sprache braucht Name + Support-URL + Privacy-URL, Fehlerbox erscheint GANZ OBEN). ⚠️ **Nach Release: OTA-Runtime = 1.31.0**; bis dahin Doppel-Publish-Regel (Version temporär auf 1.30.0 → publish → zurück).
> - **✅ CI-Rettung (12.7.):** Wächter waren 2+ Tage rot — 56 Jest-Tests (i18n ohne Provider → `TestI18nProvider`), e2e (Accept-Language ließ Landing englisch rendern → Playwright `locale:'de-DE'` gepinnt), stability (echter Prod-Fund: **medienloser Post** brach Feed-Contract → App-Guard + Web-Schedule-Guard `8fd32f9`, Post gelöscht). Seitdem alle 4 Wächter grün.
> - **✅ Verkaufs-Funnel komplett geschlossen:** `/p`-Video **stummes Autoplay** (geteilte Links zeigen sofort Bewegtbild) · **Web-Sammelbestellungs-Karte** auf `/shop/[id]` + `/shop` für ANONYME WhatsApp-Neukunden (anon-RPC `get_active_preorder_round_public`, datensparsam ohne Teilnehmer-Namen; Migration ✅; anon live verifiziert) · **„Verlängern"-Knopf** in der App-Runden-Verwaltung (RPC `reschedule_preorder_round` ✅, Samstags-Presets — kein SQL mehr nötig) · Fireside-Deadline auf **Sa 25.07.** verlängert. WhatsApp-Anschreiben de/ru (persönlich + Gruppe + Bruder-Referral) wurden formuliert — liegen im Session-13-Chat, bei Bedarf neu generieren.
> - **✅ WOZ-KOMPLETTAUDIT — alle 4 Lücken geschlossen:** (1) 🔴 **Live-Leak**: `live_sessions_select USING(true)` hebelte WOZ-Policy per OR aus (Memory `vibes-rls-permissive-or-falle`) → Migration ✅ + `/live/[id]`-Guard + **livekit-token SEC 3b** (Viewer-Token für WOZ-Streams nur für Verifizierte; Function deployt). Anon-Probe `[]` verifiziert. (2) WOZ-Badge auf Web-Profil nur noch eingeloggt. (3) **Zugang: Selbstdeklaration → Antrag → Admin-Freigabe** (Migration `20260716130000` ✅): Trigger sperrt `women_only_verified` gegen Client-Writes, RPCs request/approve/reject/revoke/leave, Tabelle `women_only_requests`, **Freigabe-Queue `/admin/women-only`**, Grandfathering. **E2E auf Gerät bestätigt** (alte App-Version wurde vom Trigger geblockt = Beweis). Memory `vibes-woz-approval-model`. (4) Web-Suche filtert women_only hart. **Stufe 2 (Community-Bürgschaft) bewusst später.**
> - **✅ WOZ-Redesign App** (Hub/Sheet/Blur): Regenbogen-Gradients + Emoji-Icons raus → Theme-Flächen, EIN Rose-Akzent (`colors.accent.rose`), monochrome Lucide-Icons, ehrliche Freigabe-Copy, i18n `woz`-Namespace de/ru/en; Leerzustand mit Schutz-Banner + „Post erstellen"-CTA. Gate zeigt Pending-Status.
> - **✅ Admin professionalisiert + Echtheits-Audit:** 103 ASCII-Umlaute repariert, Titel „Serlo Command Center", Dev-Hinweise-Panel + leerer Regionen-Karten-Platzhalter raus, Branding Serlo Operations; Header-Attrappen echt verdrahtet (24h-Chip raus, Glocke → Reports-Link mit echtem Badge, totes Menu-Icon raus). **Audit-Ergebnis: alle Dashboard-Daten ECHT** (jede Kachel bis zur DB-Quelle verifiziert). Apple-Testmeldungen (@review, 7.7.) abgearbeitet → Moderation grün.
> - **✅ ce-Tschetschenisch gestartet:** Fallback-Kette **ce→ru→de** in App+Web (Resolver kann Ketten, `fallbackChainFor()`, Jest-fixiert) — ce-Nutzer sehen Russisch statt Deutsch für Fehlendes. **Batch 1 = 218 KI-Entwürfe** in der Excel (`~/Desktop/Serlo-Tschetschenisch-Uebersetzung.xlsx`, Spalte F gelb, + ru-Quellspalte E für alle 1326 Zeilen, Prüfer-Hinweise G, Palotschka/Platzhalter maschinell validiert). **Workflow: Muttersprachler prüft F → kopiert Freigegebenes nach D („Нохчийн") → NUR Spalte D wird in ce.ts übernommen.** Web-ce hat ~300 alte ungeprüfte v0-Strings → in Review-Runde 2.
> - **✅ Landing-Adler:** drei thermik-segelnde Adler mit echtem Flugmodell (`hero-eagle.tsx`: Bank als einzige Steuergröße, R=V²/(g·tanφ), Thermik-Kreisen+Steigen, Gleitzahl 12, Winddrift, Pinhole-Projektion; asynchron durch eigene Fahrt/Revier/Drehrichtung). **Sichtfenster-bewusst** (Mobile-Sonnen-Crop, ResizeObserver) — puppeteer-verifiziert Desktop+390px je 12/12 sichtbar, Tempo 31–62 px/s.
> - **✅ PWA-Offline-Fix (Android/Foldables):** SW zeigte bei EINEM transienten Fetch-Fehler (Funkmodem-Aufwachen beim Aufklappen) sofort offline.html → jetzt 900ms + 1 Retry davor; offline.html heilt sich selbst (online-Event + 3s-Probe). CACHE_VERSION v4.
> - **✅ Nebenbei:** Monitoring-Spickzettel als PDF auf Desktop (`~/Desktop/Serlo-Monitoring-Spickzettel.pdf`) · WOZ-Freigabe von @zaur erteilt · Thumbnail-Backfill geheilt.
...
- **Stand:** App live im App Store, v1.31.0 in Apple-Prüfung, i18n de/ru komplett + ce im Review, WOZ sicherheits- und design-fertig, Admin echt+professionell, CI komplett grün, **Verkaufs-Funnel technisch geschlossen** (WhatsApp → Autoplay-Video → Runden-Karte → Vorbestellung). **Es gibt keinen technischen Blocker — der Engpass ist Verkauf** (Fireside 1/80, läuft bis Sa 25.07.).
- **🔴 OFFENE PUNKTE (nach Priorität):**
  1. **VERKAUFEN — WhatsApp-Offensive Fireside** (Deadline Sa 25.07.!): persönliche Nachrichten + Gruppen (Vorlagen de/ru im Session-13-Chat formuliert, bei Bedarf neu generieren), Bruder mit Referral-Link (`serlo.ch/i/<username>`), 2–3 kurze Videos, wöchentliches Live ankündigen. Kennzahl-Tor: 20+ Flaschen, 30 Installs. Versand-Trick: Video-DATEI + Link in EINER Nachricht (Video spielt inline, Link unfurlt).
  2. **ce-Batch-1-Review:** Excel Spalte F (218 Entwürfe) prüfen lassen → Freigegebenes nach Spalte D → dann Übertrag in `ce.ts` (App+Web) + OTA; danach Batch 2 (profile/settings/shop, ~340). Zeichen-Regeln: Palotschka Ӏ (U+04C0), `{var}` unverändert.
  3. **Build-292-Status prüfen** (ASC): seit 12.7. „Warten auf Prüfung" — bei >7 Tagen Apple-Kontakt erwägen. Nach Release: Gerätesprache-Test (Systemsprache ru → App startet ru) + ru-Store-Seite checken; **OTA-Runtime wird 1.31.0**.
  4. **Google-Branding-Prüfung** (läuft seit 10.7., kann 5–7 Tage dauern): Google Auth Platform → Überprüfungscenter → danach zeigt Login „Weiter zu Serlo".
  5. **Anon-Health-RPCs härten:** 8 Health-Snapshot-RPCs sind anon-ausführbar (MAU/Kosten öffentlich) — bewusst, weil CI-Skripte den anon-Key nutzen. Fix: RPCs auf Service-Role + CI auf Service-Key-Secret (braucht neues GitHub-Secret von Zaur).
  6. **Kleinkram Zaur:** UptimeRobot-Monitore auf www.serlo.ch umstellen · 339 alte Unread-Notifications = Datenmüll (optional aufräumen).
  7. **Später/an Bedingungen geknüpft:** Stripe Live (Anwalt AGB/Widerruf) · Supabase Custom Domain `auth.serlo.ch` (~10 $/Mon, `docs/auth-setup.md` §5, ⚠️ storageKey-Falle) · Coin-Shop-Reaktivierung (IAPs + Flag + Receipt-Verify) · **WOZ Stufe 2 Community-Bürgschaft** (wenn vertrauenswürdiger Frauen-Kern da) · Marktplatz Phase 2 (Connect/KYC) · Telegram-Alert-Token rotieren · `bunny-webhook` in config.toml prüfen · Resend/E-Mail (Android-Signup) · `schema_live.sql` neu dumpen (Policies von 16.7. fehlen im Dump).
...
| **Letzter Commit** | `195cd74` — PWA-Offline-Fix. Session-13-Kette: `783b0a1` Autoplay · `8fd32f9` Medien-Guard · `9306508` ce-Fallback · `339a6cd` Web-Runden-Karte · `f1aaad1` Verlängern · `6669aff` WOZ-Live-Fix · `621e206` WOZ-Freigabe · `4642081` Admin+WOZ-Polish · `cae13e9` 3 Adler. Alle gepusht + `ls-remote`-verifiziert. |
...
| **App-i18n** | ✅ **de/ru komplett** + **en voll** · **ce: Fallback-Kette ce→ru→de LIVE** + 218 Entwürfe in der Excel (Batch 1, wartet auf Muttersprachler-Review; nur Spalte D wird übernommen). |
| **Push** | ✅ **Lokalisiert de/ru** — `profiles.locale` (App synct automatisch) + `send-push-notification` (deployt 10.7.). |
| **Letzter OTA** | 17.7.: „WOZ Leerzustand + CTA" — wie alle Session-13-OTAs als **Doppel-Publish auf Runtime 1.30.0 UND 1.31.0** (bis Build 292 released ist). Greift beim 2. Kaltstart. |
| **Edge Functions deployed** | Wie Session 10/11 **plus**: `send-push-notification` (10.7.) + **`livekit-token` (16.7., WOZ-SEC-3b: Viewer-Token für Frauen-Streams nur für Verifizierte)**. Deploy: `npx supabase functions deploy <name> --project-ref llymwqfgujwkoxzqxrlm`. |
| **DB-Migrationen** | ✅ **ALLE ausgeführt** — neu Session 13 (alle 16.7., alle verifiziert): `…100000_live_sessions_woz_rls_fix` · `…110000_active_preorder_round_public` · `…120000_reschedule_preorder_round` · `…130000_women_only_approval`. ⚠️ `schema_live.sql`-Dump veraltet → neu dumpen. |
| **GERADE FERTIG (Session 13)** | Binary 292 + ru-Store-Listing eingereicht · CI-Rettung + Medien-Guard · Verkaufs-Funnel (Autoplay, Web-Runden-Karte, Verlängern-Knopf, Fireside 25.07.) · WOZ-Audit 4/4 zu + Freigabe-Modell + Redesign · Admin echt + professionell · ce-Kette + Batch 1 · 3 Adler · PWA-Offline-Fix. |
| **🔴 NÄCHSTE AUFGABE** | **(1) VERKAUFEN** — WhatsApp-Offensive Fireside bis Sa 25.07. (Funnel steht komplett). **(2)** ce-Batch-1-Review → Übertrag in ce.ts. **(3)** Build-292- + Google-Branding-Status prüfen. |
| **Monitoring** | Bewacht (UptimeRobot, Sentry App+Web, Telegram-CI, Stripe-Mails). **Alle 4 CI-Wächter grün** (seit 16.7.), Root-tsc 0 · Web-tsc 0 · Jest 135+296 · e2e 4/4 · Moderation grün (Apple-Testmeldungen abgearbeitet). ⚠️ Playwright braucht `locale: 'de-DE'`. UptimeRobot-Monitore noch auf serlo-web.vercel.app → auf www.serlo.ch umstellen (Zaur, 2 Min). Spickzettel: `docs/MONITORING.md` + PDF auf Desktop. |
