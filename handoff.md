...
- **🔴 OFFENE PUNKTE (nach Priorität):**
  ...
  7. 🔴 **OFFENE MIGRATION `20260716130000_women_only_approval`** (Zaur ausführen!) — WOZ-Zugang von Selbstdeklaration → **Antrag+Admin-Freigabe** (Commit `621e206`, OTA beide Runtimes raus). ⚠️ **REIHENFOLGE:** Web deployt automatisch (nutzt neue RPCs); App-OTA greift beim 2. Kaltstart. Nach der Migration schlägt ein „Beitreten" aus der ALTEN App-Version (vor OTA) am DB-Trigger fehl (soft error) — egal, es gibt eh nur 2 Test-WOZ-Accounts (grandfathered). **Freigabe-Queue: `/admin/women-only`.** Prüfen nach Migration: Anon-Call `request_women_only` → permission denied; direkter `profiles.update({women_only_verified:true})` → Trigger-Exception.
  8. **Später/an Bedingungen geknüpft:** Stripe Live (wartet auf Anwalt für AGB/Widerruf) · Supabase Custom Domain `auth.serlo.ch` für den iOS-Login-Dialog (~10 $/Mon, Checkliste `docs/auth-setup.md` §5, ⚠️ storageKey-Falle) · Coin-Shop-Reaktivierung (IAPs neu in ASC + Flag + Receipt-Verify) · **WOZ Stufe 2 optional: Community-Bürgschaft** (verifizierte Frauen bürgen für Neue — später, wenn vertrauenswürdiger Kern da; heute admin-approval korrekt) · Marktplatz Phase 2 (Connect/KYC) · Kleinkram: Telegram-Alert-Token rotieren, `bunny-webhook` in config.toml prüfen, Resend/E-Mail (Android-Signup).
