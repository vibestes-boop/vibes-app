...
- **🔴 OFFENE PUNKTE (nach Priorität):**
  ...
  6. 🔴 **OFFENE MIGRATION `20260716120000_reschedule_preorder_round`** (Zaur ausführen) — neue RPC für den **„Verlängern"-Knopf in der App** (Runden-Verwaltung: Mein Shop → 📦-Icon → fulfillment; Commit `f1aaad1`, OTA auf beide Runtimes 1.30.0+1.31.0 raus). Bis die Migration läuft, zeigt der Knopf beim Tippen einen „nochmal probieren"-Alert (soft fail). Damit muss man die Deadline nicht mehr per SQL ändern.
  6. **Später/an Bedingungen geknüpft:** Stripe Live (wartet auf Anwalt für AGB/Widerruf) · Supabase Custom Domain `auth.serlo.ch` für den iOS-Login-Dialog (~10 $/Mon, Checkliste `docs/auth-setup.md` §5, ⚠️ storageKey-Falle) · Coin-Shop-Reaktivierung (IAPs neu in ASC + Flag + Receipt-Verify) · WOZ-Entscheidungen (öffentliches Frauen-Badge auf Web-Profil? · Bürgschafts-Verifizierung Stufe 2?) · Marktplatz Phase 2 (Connect/KYC) · Kleinkram: Telegram-Alert-Token rotieren, `bunny-webhook` in config.toml prüfen, Resend/E-Mail (Android-Signup).
