...
11. **Spalten-REVOKE friert die Spaltenliste ein.** ⚠️ **Diese Liste war bis zum 23.08.2026 an beiden Enden falsch.** Am echten Abzug gemessen sind es **fünf** Tabellen: **`live_auctions`, `live_sessions`, `product_orders`, `profiles`, `scheduled_lives`** — darunter zwei, an denen ständig gearbeitet wird. `user_whip_ingresses` gehört seit `20260823100000` **nicht mehr** dazu. Die Prüfung geht über `pg_attribute.attacl`, NICHT über `information_schema.column_privileges`: Letztere fächert auch Rechte auf, die aus dem Tabellen-Recht folgen, und meldet damit jede normale Tabelle als eingefroren (die Gegenprobe in `20260822140000` ist genau deshalb unbrauchbar).
    ```sql
    SELECT a.attname FROM pg_attribute a
     WHERE a.attrelid = 'public.<tabelle>'::regclass
