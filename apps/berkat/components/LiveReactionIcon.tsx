// Die Reaktions-Symbole — reines SVG statt Systemschrift.
//
// Übernommen aus Serlo (`components/live/LiveReactionIcon.tsx`). Bewusst kopiert
// und nicht quer importiert: Berkat hat eigene `node_modules` und eine eigene
// Metro-Auflösung, ein Pfad aus dem Projekt heraus wäre genau die Sorte
// Verkabelung, die später still bricht. **Wer die Symbole ändert, muss beide
// Stellen anfassen.**
//
// Warum überhaupt kein Emoji: Ein Emoji ist die Schrift des Geräts, nicht die
// eigene Marke — es sieht auf jedem Hersteller anders aus, und wo die
// Emoji-Schrift fehlt, erscheint ein leeres Kästchen (im iOS-Simulator zu
// sehen). Als SVG sieht es überall gleich aus.
//
// Alle fünf sind hier, obwohl Berkat nur Herzen SENDET: Der Broadcast-Kanal ist
// derselbe wie in Serlo, und von dort können Flamme, Lachen, Klatschen und Hoch
// hereinkommen. Unbekanntes fällt weiterhin auf Text zurück, damit ein späteres
// sechstes Symbol nicht verschwindet.
...
const HeartIcon = memo(({ size }: { size: number }) => (
...
const FireIcon = memo(({ size }: { size: number }) => (
  <Svg width={size} height={size} viewBox="0 0 100 100">
    <Defs>
...
const LaughIcon = memo(({ size }: { size: number }) => (
  <Svg width={size} height={size} viewBox="0 0 100 100">
    <Defs>
    ...
    <Path d="M24 42 Q32 32 40 42" stroke="#1A1A1A" strokeWidth="4" strokeLinecap="round" fill="none" />
    <Path d="M60 42 Q68 32 76 42" stroke="#1A1A1A" strokeWidth="4" strokeLinecap="round" fill="none" />
    <Path d="M26 58 Q50 88 74 58 Q62 64 50 64 Q38 64 26 58 Z" fill="#1A1A1A" />
    <Path d="M38 68 Q50 80 62 68 Q56 76 50 76 Q44 76 38 68 Z" fill="#FF5B82" />
    <Path d="M18 52 Q14 58 16 64 Q22 62 22 56 Q20 54 18 52 Z" fill="url(#tearGrad)" />
    <Path d="M82 52 Q86 58 84 64 Q78 62 78 56 Q80 54 82 52 Z" fill="url(#tearGrad)" />
  </Svg>
));
LaughIcon.displayName = 'LaughIcon';

const ClapIcon = memo(({ size }: { size: number }) => (
  <Svg width={size} height={size} viewBox="0 0 100 100">
    <Defs>
...
const RaiseIcon = memo(({ size }: { size: number }) => (
  <Svg width={size} height={size} viewBox="0 0 100 100">
    <Defs>
...
/** Zeichnet das passende Symbol zu einem Broadcast-Zeichen. */
export const LiveReactionIcon = memo(function LiveReactionIcon({
  emoji,
  size = 36,
}: {
  /** Das Zeichen, wie es über den Broadcast-Kanal ankommt. */
  emoji: string;
  size?: number;
}) {
  switch (emoji) {
    // Mit und ohne Variantenzeichen (U+FE0F) — je nach Absender kommt beides an.
    case '❤️':
    case '❤':
