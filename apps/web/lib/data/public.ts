...
//     share_count           → existiert nicht → 0-Default
//
//   comments:
//     body                  → text
...
export const getProfilePosts = cache(
  async (userId: string, limit = 24, before?: string, sort: ProfileSortKey = 'newest'): Promise<Post[]> => {
    ...
    let query = supabase
      .from('posts')
      .select(
        `id, author_id, caption, media_url, thumbnail_url, view_count, tags, allow_comments, allow_duet, women_only, is_pinned, aspect_ratio, created_at,
...
export const getPost = cache(async (postId: string): Promise<PostWithAuthor | null> => {
  ...
  const { data, error } = await supabase
    .from('posts')
    .select(
      `id, author_id, caption, media_url, media_type, thumbnail_url, view_count, tags, allow_comments, allow_duet, allow_download, privacy, women_only, aspect_ratio, audio_url, audio_volume, created_at,
       like_count:likes(count),
       comment_count:comments(count),
...
export const getProfileReposts = cache(async (userId: string, limit = 48): Promise<Post[]> => {
  ...
  const { data, error } = await supabase
    .from('posts')
    .select(
      `id, author_id, caption, media_url, thumbnail_url, view_count, tags, allow_comments, allow_duet, aspect_ratio, created_at,
       like_count:likes(count),
       comment_count:comments(count)`,
    )
    .in('id', postIds);
