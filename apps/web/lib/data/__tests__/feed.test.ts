...
// -----------------------------------------------------------------------------
// getMyFollowedAccounts — v1.w.UI.212 coverage fix
// -----------------------------------------------------------------------------

describe('getMyFollowedAccounts', () => {
  it('returns [] when not signed in', async () => {
    setupSupabase({ auth: { user: null } });
    const result = await getMyFollowedAccounts();
    expect(result).toEqual([]);
  });

  it('returns [] when the user follows nobody', async () => {
    setupSupabase({
      auth: { user: { id: 'me' } },
      tables: {
        follows: { data: [], error: null },
  ...
  it('returns followed accounts in follow-date order', async () => {
    setupSupabase({
      auth: { user: { id: 'me' } },
      tables: {
        follows: {
          data: [
            { following_id: 'u2', created_at: '2026-04-20T12:00:00Z' },
            { following_id: 'u1', created_at: '2026-04-18T12:00:00Z' },
          ],
          error: null,
        },
        profiles: {
          data: [
            ...
            { id: 'u2', username: 'bob',   display_name: 'Bob',   avatar_url: null, verified: true },
          ],
          error: null,
        },
      },
    ...
    const result = await getMyFollowedAccounts();
    // follow-date order: u2 first (newer follow), then u1
    expect(result).toHaveLength(2);
  ...
  it('silently drops follows whose profile was deleted', async () => {
    setupSupabase({
      auth: { user: { id: 'me' } },
      tables: {
        follows: {
          data: [
            { following_id: 'deleted-user', created_at: '2026-04-20T12:00:00Z' },
            { following_id: 'u1',           created_at: '2026-04-18T12:00:00Z' },
          ],
          error: null,
        },
        // profiles only returns u1 — deleted-user's row is gone
        profiles: {
          data: [
            { id: 'u1', username: 'alice', display_name: null, avatar_url: null, verified: null },
          ],
          error: null,
        },
      },
...
// -----------------------------------------------------------------------------
// getPostsByTag — v1.w.UI.212 coverage fix
// -----------------------------------------------------------------------------

describe('getPostsByTag', () => {
  it('returns [] for empty tag', async () => {
    setupSupabase({ auth: { user: null } });
  ...
  it('strips leading # from tag', async () => {
    ...
    await getPostsByTag('#vibes');

    const postsChain = client.from.mock.results.find(
      (_r, i) => (client.from.mock.calls[i] as unknown[])[0] === 'posts',
    )?.value as { contains: jest.Mock };
    // The function lowercases and strips # → should call contains with ['vibes']
    expect(postsChain.contains).toHaveBeenCalledWith('tags', ['vibes']);
  });
  ...
  it('returns [] when query errors', async () => {
    setupSupabase({
      auth: { user: null },
      tables: {
        posts: { data: null, error: { code: 'PGRST301', message: 'error' } },
      },
    });

    const result = await getPostsByTag('vibes');
    expect(result).toEqual([]);
  });

  it('normalizes rows for anonymous viewer', async () => {
    const rawPost = {
      id: 'tag-p1',
      author_id: 'author-1',
      caption: 'tagged post',
