  ...
  return (
    <div className="space-y-4">
      <section className="flex flex-col gap-2 lg:flex-row lg:items-end lg:justify-between">
        <div>
          <h1 className="text-xl font-bold text-slate-950">Regionale Aktivitaet</h1>
      ...
      {roles.can_admin && (
        <section className="rounded-lg border border-slate-200 bg-white p-3 shadow-sm">
          <div className="mb-3 flex items-center gap-2">
            ...
            <div>
              <h2 className="text-sm font-bold text-slate-950">Region-Metriken eintragen</h2>
              <p className="text-[11px] text-slate-500">Upsert pro ISO-2-Land, Datum und Quelle. Keine Standortdaten werden geraten.</p>
            </div>
          </div>
          <form action={refreshProfileRegions} className="mb-3 flex flex-wrap items-center justify-between gap-2 rounded-md border border-blue-100 bg-blue-50 px-3 py-2">
          ...
          <form action={upsertRegionMetrics} className="grid gap-2 lg:grid-cols-[0.45fr_1fr_0.8fr_repeat(6,0.65fr)_0.75fr_auto]">
            <Field label="Code">
              <input
                ...
                maxLength={2}
                placeholder="DE"
                className="h-9 w-full rounded-md border border-slate-200 px-2.5 text-xs uppercase outline-none focus:border-blue-400 focus:ring-2 focus:ring-blue-100"
              />
            </Field>
            <Field label="Land">
              <input
              ...
              <tbody>
                {metrics.map((metric) => (
                  <tr key={metric.id} className="border-b border-slate-100 last:border-0">
                    <td className="py-2 pr-3 font-semibold text-slate-800">{metric.country_name} <span className="text-slate-400">({metric.country_code})</span></td>
                    <td className="py-2 pr-3 tabular-nums text-slate-600">{metric.metric_date}</td>
                    <td className="py-2 pr-3 text-right tabular-nums text-slate-700">{formatNumber(metric.active_users)}</td>
                    <td className="py-2 pr-3 text-right tabular-nums text-slate-700">{formatNumber(metric.total_profiles)}</td>
                    <td className="py-2 pr-3 text-right tabular-nums text-slate-700">{formatNumber(metric.new_registrations)}</td>
                    <td className="py-2 pr-3 text-right tabular-nums text-slate-700">{formatNumber(metric.posts)}</td>
                    <td className="py-2 pr-3 text-right tabular-nums text-slate-700">{formatNumber(metric.views)}</td>
                    <td className="py-2 pr-3 text-right tabular-nums text-slate-700">{formatNumber(metric.reports)}</td>
                    <td className="py-2 text-right text-slate-500">{metric.source}</td>
                  </tr>
                ))}
              </tbody>
            </table>
      ...
      <section className="rounded-lg border border-slate-200 bg-white p-3 shadow-sm">
        <div className="mb-2 flex items-center gap-2">
          ...
          <h2 className="text-xs font-bold text-slate-950">Naechste Stufe</h2>
        </div>
