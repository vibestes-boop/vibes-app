...
BEGIN
  ...
  IF NULLIF(v_service_role_key, '') IS NULL OR NULLIF(v_project_url, '') IS NULL THEN
    RAISE NOTICE 'Missing app.settings.project_url/service_role_key; r2-delete-queue cron not scheduled';
    RETURN;
  END IF;
